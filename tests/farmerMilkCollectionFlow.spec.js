...
async function verifyLoginPageAndEnterCredentials(page) {
  ...
  if (!username || !password) throw new Error('USERNAME or PASSWORD env var not set!');
...
test('HitechDairy – Login + Farmer Milk Collection Flow', async ({ page }) => {
  ...
  try {
    ...
    let financialYearLoaded = false;
    page.on('request', request => {
      if (request.url().includes('MasterDropdown/GetFinancialYear')) {
        financialYearLoaded = false;
      }
    });
    page.on('response', response => {
      if (response.url().includes('MasterDropdown/GetFinancialYear')) {
        financialYearLoaded = true;
      }
    });

    async function ensureFinancialYearLoaded() {
      if (financialYearLoaded) return;
      console.log('Waiting for GetFinancialYear API response before CAPTCHA...');
      await page.waitForResponse(res => res.url().includes('MasterDropdown/GetFinancialYear'), { timeout: 15000 }).catch(() => {});
      financialYearLoaded = true;
    }
      ...
      try {
        ...
        for (let captchaAttempt = 1; captchaAttempt <= 3; captchaAttempt++) {
          try {
            ...
            // Ensure API is loaded before dealing with CAPTCHA
            await ensureFinancialYearLoaded();
            ...
            await page.waitForURL('**/dashboard', { timeout: 15000 }).catch(async () => {
              // Fallback: if we're no longer on the login page, consider it success
              const currentUrl = page.url();
              if (!currentUrl.includes('login')) {
                console.log(`✅ Login detected via URL check: ${currentUrl}`);
              } else {
